...
export default function EditProductModal({ isOpen, onClose, onProductUpdated, product }: EditProductModalProps) {
  ...
  const updatePriceStatus = (price: number, recommended: number) => {
    if (price > recommended) {
      ...
      setPriceBadge(<span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 ml-2">Vyššia</span>);
    } else if (price < recommended) {
      setPriceColor('text-green-600');
      setPriceMessage(`Najnižšia nová cena bude ${price} €`);
      setPriceBadge(<span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 ml-2">Najnižšia</span>);
    } else {
